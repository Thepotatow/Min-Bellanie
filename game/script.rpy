...
init python:
    class Camera:
        def __init__(self, name, icon, camera_feed):
            self.name = name
            self.icon = icon
...
label script_FB:
    ...
    t "Om det är okej för henne så hade det varit perfekt. Pappa har en ritning över huset. Jag kan komma över med den ikväll så Kendall kan titta på det."

    c "Absolut. Jag säger till henne"

    scene black 
    with fade

    #Ljudeffekter, hjärtslag, dörr som öppnas och stängs, häftiga andetag, fullkomlig tystnad. 
    c "Mamma? Mamma, kan du höra mig? Kan du prata med mig, mamma? Snälla!...."

    #Dörrljud. Tystnad. 

    c "Pappa?... Vad har du gjort? Vad är det där?... Pappa!"

    jump script_13b


label script_13b:
    #Mammas kontor framför skrivbordet. 

    c "Hon måste ha kvar ritningen nånstans och om jag kan hitta den så kan jag ha koll på Bellanie och lära mig vad jag borde säga och göra för att få henne att säga ja. I slutändan måste hon säga ja. Jag behöver bara lära mig hur hon fungerar."
